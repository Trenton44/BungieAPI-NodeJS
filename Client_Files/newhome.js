...
function slotController(){
  ...
  this.updateInventory = async function(){
  ...
  this.show = function(bool){
    for(i in this.slots){
      for(z in this.slots[i]){
        console.log("Updating display status of: ");
        console.log(this.slots[i][z]);
...
function Item(){
  ...
  this.show = function(bool){
    if(bool){ console.log(this.element); this.parentElement.append(this.element); }
    else{ console.log(this.element);this.element.remove();}
  };
};
